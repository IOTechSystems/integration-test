...
docker-compose run --rm postman run ${COLLECTION_PATH} \
    ...
    --reporters="junit,cli" --reporter-junit-export "newman/supportScheduler_interval_`date "+%Y%m%d-%H%M%S"`.xml"


docker-compose run --rm postman run ${COLLECTION_PATH} \
    --folder="intervalAction" --iteration-data="data/intervalActionData.json" --environment=${ENV_PATH} \
    --reporters="junit,cli" --reporter-junit-export "newman/supportScheduler_intervalAction_`date "+%Y%m%d-%H%M%S"`.xml"
