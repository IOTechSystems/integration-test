...
docker-compose run --rm postman run ${COLLECTION_PATH} \
    ...
    --reporters="junit,cli" --reporter-junit-export "newman/coreMetadata_schedule_`date "+%Y%m%d-%H%M%S"`.xml"

docker-compose run --rm postman run ${COLLECTION_PATH} \
    --folder="schedule_error_4xx" --iteration-data="data/scheduleData.json" --environment=${ENV_PATH} \
    --reporters="junit,cli" --reporter-junit-export "newman/coreMetadata_schedule4xx_`date "+%Y%m%d-%H%M%S"`.xml"
...
docker-compose run --rm postman run ${COLLECTION_PATH} \
    ...
    --reporters="junit,cli" --reporter-junit-export "newman/coreMetadata_scheduleevent_`date "+%Y%m%d-%H%M%S"`.xml"

docker-compose run --rm postman run ${COLLECTION_PATH} \
    --folder="scheduleevent_error_4xx" --iteration-data="data/scheduleEventData.json" --environment=${ENV_PATH} \
    --reporters="junit,cli" --reporter-junit-export "newman/coreMetadata_scheduleevent4xx_`date "+%Y%m%d-%H%M%S"`.xml"
