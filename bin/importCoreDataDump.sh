...
if [ -f $EVENTDATADUMP ]; then
    ...
    docker-compose exec -T mongo /bin/bash -c "mongoimport -d coredata -c event --file eventDb.json"
...
if [ -f $READINGDATADUMP ]; then
    ...
    docker-compose exec -T mongo /bin/bash -c "mongoimport -d coredata -c reading --file readingDb.json"
...
if [ -f $VDDATADUMP ]; then
    ...
    docker-compose exec -T mongo /bin/bash -c "mongoimport -d coredata -c valueDescriptor --file valueDescriptorData.json"
